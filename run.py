...
from segmentation_module_addprototype import make_model, make_model_v2
...
def main(opts):
    ...
    print(f"Backbone: {opts.backbone}")
    ...
    if opts.step > 0:
        ...
        if (not os.path.exists(path)):  # and opts.name != 'EWC' and opts.name != 'MiB' and opts.name != 'PI' and opts.name != 'RW':
            if opts.task == '19-1' or opts.task == '15-5' or opts.task == '10-10':
                ...
                try:
                    copy(path_FT, path)
                    copy('./train.py',os.path.join(path.split('//')[0],'train.py'))
                    copy('./run.py', os.path.join(path.split('//')[0], 'run.py'))
                    copy('./argparser.py', os.path.join(path.split('//')[0],'argparser.py'))
                    copy('./segmentation_module_addprototype.py', os.path.join(path.split('//')[0], 'segmentation_module_addprototype.py'))
    ...
    old_epoch_prototype_flag = False
    # old_epoch_prototype = torch.zeros([sum(tasks.get_per_task_classes(opts.dataset,opts.task, opts.step-1)), opts.feat_dim])
    if opts.step > 0 and (opts.loss_de_prototypes > 0 or opts.lfc_sep_clust > 0 or opts.loss_fc):
        ...
        old_count_features = count_features_old
        old_count_features.requires_grad = False
        old_count_features = old_count_features.to(device)
        old_epoch_prototype = prototypes_old
        old_epoch_prototype.requires_grad = False
        old_epoch_prototype = old_epoch_prototype.to(device)
        # old_epoch_prototype_flag =True
        del step_checkpoint
    if opts.step > 0 and opts.featupdate_loss:
        old_epoch_prototype_flag = True

    logger.info(f"Current prototypes are {prototypes}")
    logger.info(f"Current count_features is {count_features}")
    print(f"Current prototypes are {prototypes}")
    print(f"Current count_features is {count_features}")
    ...
    while cur_epoch < opts.epochs and TRAIN:
        # =====  Train  =====
        model.train()
        if old_epoch_prototype_flag:
            # if opts.update_background>0 and cur_epoch==0:
            #     prototypes[0, ...] = torch.zeros([1, 2048]).to(device)
            #     count_features[0] = torch.zeros(1, dtype=torch.long).to(device)
            if cur_epoch>0:
                if opts.featupdate_count:
                    prototypes[1:prototypes_old.shape[0], :] = old_epoch_prototype[1:prototypes_old.shape[0],:]
                    count_features[1:count_features_old.shape[0]] = old_count_features[1:count_features_old.shape[0]]
                    # old_count_features = torch.zeros([count_features_old.shape[0]])
                    # old_count_features.requires_grad = False
                    # old_count_features = old_count_features.to(device)
                    # old_epoch_prototype = torch.zeros([prototypes_old.shape[0], opts.feat_dim])
                    # old_epoch_prototype.requires_grad = False
                    # old_epoch_prototype = old_epoch_prototype.to(device)
                    print(f"Current prototypes are {prototypes}")
                    logger.info(f"Current prototypes are {prototypes}")
                    print(f"Current counts are {count_features}")
                    logger.info(f"Current counts are {count_features}")
                    print(f"Current old epoch prototypes are {old_epoch_prototype}")
                    logger.info(f"Current old epoch prototypes are {old_epoch_prototype}")
                    print(f"Current old counts are {old_count_features}")
                    logger.info(f"Current old counts are {old_count_features}")
                else:
            ...
            epoch_loss, prototypes, count_features, old_epoch_prototype, old_count_features = trainer.train(cur_epoch=cur_epoch, optim=optimizer, world_size=world_size,
                                       train_loader=train_loader, scheduler=scheduler, logger=logger,
                                       print_int=opts.print_interval, prototypes=prototypes, count_features=count_features, label2color=label2color,
                                                       denorm=denorm, old_epoch_prototype=old_epoch_prototype, old_count_features= old_count_features)

        else:
            epoch_loss, prototypes, count_features = trainer.train(cur_epoch=cur_epoch,
                                                                                        optim=optimizer,
        ...
        print(f"End of Epoch {cur_epoch + 1}/{opts.epochs}, Average Loss={epoch_loss[0] + epoch_loss[1]},"
                    f" Class Loss={epoch_loss[0]}, Reg Loss={epoch_loss[1]}")
        # print("E-Loss", epoch_loss[0] + epoch_loss[1], cur_epoch)
        # print("E-Loss-reg", epoch_loss[1], cur_epoch)
        # print("E-Loss-cls", epoch_loss[0], cur_epoch)
    ...
    if opts.test:
        # keshihua
        vis_dir = "/workspace/visual/test/"
        # val_loss, val_score, _ = trainer.validate(loader=test_loader, metrics=val_metrics, logger=logger,
        #                                           world_size=world_size, vis_dir=logdir_full,test_dir=vis_dir, label2color=label2color,
        #                                           denorm=denorm)
        # val_loss, val_score, _ = trainer.validate(loader=test_loader, metrics=val_metrics, logger=logger,prototypes=prototypes,
        #                                           world_size=world_size, vis_dir=vis_dir, label2color=label2color, denorm=denorm)
        val_loss, val_score, _ = trainer.validate(loader=test_loader, metrics=val_metrics, logger=logger,
                                                  world_size=world_size,label2color=label2color, denorm=denorm)
