...
def vaihingen_cmap():
    cmap = np.zeros((256, 3), dtype=np.uint8)
    colors = [[255, 0, 0], [0, 0, 255],[0, 255, 0],[255, 255, 255],[0, 255, 255], [255, 255, 0]]
    for i in range(len(colors)):
        cmap[i] = colors[i]
    return cmap.astype(np.uint8)
...
def bisai_cmap():
    cmap = np.zeros((256, 3), dtype=np.uint8)
    colors = [[0,0,0],[188, 162, 137], [0, 255, 0], [0, 0, 255], [255, 255, 255],[255,255,0],[84,130,53],[200,170,125],[0,255,255]]
    for i in range(len(colors)):
        cmap[i] = colors[i]
    return cmap.astype(np.uint8)
...
def ISAID_cmap():
    cmap = np.zeros((256, 3), dtype=np.uint8)
    colors = [[0, 0, 0],[0, 63, 0],[0, 63, 191],[0, 127, 63],[0, 63, 255],[0, 100, 155],[0, 0, 191],[0, 127, 127],[0, 127, 255],[0, 191, 127],
    [0, 0, 63],[0, 191, 127],[0, 127, 191],[0, 63, 63],[0, 0, 255],[0, 63, 127]]
    for i in range(len(colors)):
        cmap[i] = colors[i]
    return cmap.astype(np.uint8)
